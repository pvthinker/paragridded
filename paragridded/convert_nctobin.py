...
tilesperregion = [643, 525, 416, 475, 568, 653, 364, 285, 549, 549, 454, 450, 651]
...
def get_tar_struct(subd):
    ...
    for tarsize in size_set:
        ...
        tarfile = next((fname for fname, fsize in sizes.items() if fsize == tarsize), None)
        # reverse the name list to pick the last one
        names = reversed(list(sizes.keys()))
        tarfile = next((fname for fname in names if sizes[fname] == tarsize), None)
        print(tarfile)
...
def load_tar_struct():
    pklfile = f"{giga.dirdata}/tar_struct.pkl"
...
def extract_from_tar(date, subd, tile, quarter):
    ...
    if grid_present:
        if quarter == 0:
            ...
            skip = len(tiles)*blocks_firsttile + (len(tiles)*(quarter-1) + itile)*blocks_per_tile + 1
...
def gethisindex(date, subd):
    ...
    hisindex = [result.stdout.split("\n")[quarter].split(".")[-3] for quarter in range(4)]
    return hisindex
...
for date in hisdates:
    ...
    for subd in regions:
        ...
        toc = time.time()
        elapsed = toc-tic
        print(f"time to convert {len(tasks)} tiles with {nworkers} workers: {elapsed:.4} s")
        pool.close()
...
date = "2008-10-07"
for subd in regions:
    tasks += [(date, subd, t, quarter) for quarter in range(4) for t, s in giga.subdmap.items() if s == subd]
...
toc = time.time()
elapsed = toc-tic
print(f"time to extract all tiles from region {subd:02} with {nworkers} workers: {elapsed:.4} s")
pool.close()
...
nworkers = ndates*4
tasks = [(hisdates[i], subd, quarter) for i in range(ndates) for quarter in range(4)]
